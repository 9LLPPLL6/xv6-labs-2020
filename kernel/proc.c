...
found:
  ...
  // map a kernel stack for a process in process's kernel pagetable
  uint64 kstack_va = p->kstack;
  uint64 pa = kvmpa(kstack_va);
  kvmmap(p->kpagetable, kstack_va, pa, PGSIZE, PTE_R | PTE_W);
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        // swich process's kernel pagetable
